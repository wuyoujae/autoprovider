<template>
  <div class="agent-message-item-edit">
    <button
      class="agent-message-item-edit__button"
      type="button"
      :class="{
        'agent-message-item-edit__button--running': status === 'running',
      }"
    >
      <FilePen class="agent-message-item-edit__icon" />
      <span class="agent-message-item-edit__text">
        <span class="agent-message-item-edit__label">编辑文件</span>
        <span
          class="agent-message-item-edit__file"
          :title="fullPath"
      ...
      <ChevronRight class="agent-message-item-edit__arrow" />
    </button>
  </div>
</template>
...
import { FilePen, ChevronRight } from "lucide-vue-next";
...
const parsedPayload = computed(() => {
  ...
  try {
    ...
    console.warn("[AgentMessageItemEdit] 解析内容失败:", error);
    return null;
  }
});
...
.agent-message-item-edit {
  ...
  &__button {
    ...
    &:hover .agent-message-item-edit__text {
      color: rgba(255, 255, 255, 0.9);
    }

    &--running {
      .agent-message-item-edit__text {
