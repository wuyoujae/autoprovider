<template>
  <div class="agent-message-item-delect">
    <button
      class="agent-message-item-delect__button"
      type="button"
      :class="{
        'agent-message-item-delect__button--running': status === 'running',
      }"
    >
      <Trash2 class="agent-message-item-delect__icon" />
      <span class="agent-message-item-delect__text">
        <span class="agent-message-item-delect__label">{{ actionLabel }}</span>
        <span
          class="agent-message-item-delect__file"
          :title="fullPath"
      ...
      <ChevronRight class="agent-message-item-delect__arrow" />
    </button>
  </div>
</template>
...
const parsedPayload = computed(() => {
  ...
  try {
    ...
    console.warn("[AgentMessageItemDelect] 解析内容失败:", error);
    return null;
  }
});
...
const actionLabel = computed(() => {
  return isFolder.value ? "删除文件夹" : "删除文件";
});
...
.agent-message-item-delect {
  ...
  &__button {
    ...
    &:hover .agent-message-item-delect__text {
      color: rgba(255, 255, 255, 0.9);
    }

    &--running {
      .agent-message-item-delect__text {
