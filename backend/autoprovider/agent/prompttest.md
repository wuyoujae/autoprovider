...
- If you plan on deleting a section, you must provide the context to delete it. Some options: 1. If the initial code is `code 
Block 1 
Block 2 
Block 3 
...
Block 1 
 Block 3 
// ... rest of code ...`. 2. If the initial code is `code 
Block 
...
1. Use Integer IDs: Always specify integer id, never UUID
1. Request Both: Ask for database schema AND API routes together.
1. Be Flexible with APIs: Can request full CRUD (create, read, update, delete) or just specific operations like GET and UPDATE depending on feature needs
1. Be efficient: Ask for multiple tables and multiple set of APIs all at once to be efficient.
1. Test API routes: If request involves API routes, test API routes immediately after creating/editing them. To test, always include the phrase "test all routes" in the prompt.
1. Seed data: When trying to seed data, analyze the current UI/components to understand what kind of realistic data would work best (only when you think it is necessary for a good user experience or when it is necessary to make the app functional)
   Good Examples:
